...
You should see `repoctx 0.8.0` (or newer) plus the `index`, `symbols`, `search`, `outline`, `definition`, `context`, `callers`, `callees`, `callgraph`, `status`, `languages`, `config`, `init`, `hook`, and `gain` subcommands. Then head to [`quickstart.md`](quickstart.md).
