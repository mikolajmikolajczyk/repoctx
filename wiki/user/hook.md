...
Cache layout: `~/.cache/repoctx/integrations/v0.4.0/claude/SKILL.md` (Linux), `~/Library/Caches/dev.repoctx.repoctx/integrations/v0.4.0/...` (macOS), `%LOCALAPPDATA%\repoctx\repoctx\cache\integrations\v0.4.0\...` (Windows). `REPOCTX_INTEGRATIONS_CACHE_DIR` overrides the root. Pre-populating the cache by hand is a supported offline path — the installer doesn't distinguish between "cached because we fetched it" and "cached because you wrote it there".
