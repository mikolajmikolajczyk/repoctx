...
- **`store`** — SQLite schema, migrations, query helpers. The only module that touches the DB. Schema v5 (files, symbols, meta, usage, settings, **calls**, **imports**). The `calls` table holds name-based call edges (caller name + start line, callee name, site, `resolution`); `callers_of`/`callees_of` resolve callees to symbols by name at query time (ADR-0010). The `imports` table holds string-based import edges (file → raw module specifier, site, `resolution`); `deps_of`/`importers_of` query by file / substring (ADR-0011). `BEGIN IMMEDIATE` on migration so parallel indexers serialize cleanly.
