...
v0.5.2 shipped 2026-06-12. CLI surface complete: indexing, search, navigation, per-agent install, per-repo config, transparent Claude Code rewrite hook, language-coverage advisory. LSP daemon deferred — see [`status.md`](status.md) and the daemon epic `58b45d5` in Radicle.
...
5. **Config layer** — every persistent CLI behavior (`hook.rewrite`, `hook.ref`, `hook.no_cache`, `hook.chain_commands`, `gain.no_record`, `gain.record_query`, `output.default`) lives in a `settings` key/value table inside the same `.repoctx/index.db`. `Config::load(&Store)` resolves four sources in order — CLI flag, env var (`REPOCTX_<SECTION>_<KEY>`), settings row, built-in default — and tracks each value's `Source` for diagnostics. Schema v3 adds the `settings` table; older DBs migrate transparently. See `wiki/user/config.md`.
6. **Rewrite hook** (`hook_rewrite` + `hook_takeover`) — `repoctx hook claude` is a PreToolUse handler Claude Code calls with `tool_input.command` on stdin. The handler tries a conservative semantic rewrite (`rg <ident>`, `rg "fn <ident>"`, `grep -r/-rn` variants), and on miss executes each command in `hook.chain_commands` in order, propagating the first rewrite a chain returns or exiting 1 (silent passthrough) when all chains pass through. `repoctx hook install claude` ALWAYS takes ownership of the Bash matcher in `.claude/settings.json`: pre-existing entries are saved into `hook.chain_commands`, then a single `{"matcher":"Bash","hooks":[{"command":"repoctx hook claude"}]}` entry is inserted. `repoctx hook doctor` re-runs the takeover idempotently to recover from sibling installers (rtk reinstall, etc.). The decision doc + rule set live at `wiki/decisions/2026-06-12-rewrite-hook-design.md`.
