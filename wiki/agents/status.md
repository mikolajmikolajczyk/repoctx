...
- `repoctx deps <file>` / `rdeps <module>` (unreleased, epic #4) — import / dependency graph for the core-8 langs; ADR-0011; schema v5 `imports` table. `deps` lists a file's import specifiers; `rdeps` finds importers by substring match on the specifier (`rdeps storage-idb` → importers of `@adapters/storage-idb`). String-based, query-time resolution; empties carry an advisory. Foundation for boundary checks / cycle detection / module maps (epic #4 children).
...
- **Import / dependency graph** (`#4`, epic): `deps`/`rdeps` foundation landed (schema v5, core-8 import extraction, ADR-0011); remaining children — boundary/layering check command, import-cycle detection, module dependency map + public API surface.
