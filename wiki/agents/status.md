...
- `repoctx prime` (unreleased, issue #11) — compact ~600-token session-start orientation digest (headline + top subsystems + hubs + entry points + skill pointer), deterministic markdown. `hook install claude` registers it as a **SessionStart** hook so it primes the agent's context to use repoctx over grep/cat (adoption-via-priming, decision 2026-06-16). Never cold-indexes (nudge if unindexed); graph referenced by command, not inlined.
