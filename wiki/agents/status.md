...
- `repoctx config show/get/set/unset` — per-repo settings (`hook.rewrite`, `hook.ref`, `hook.no_cache`, `gain.no_record`, `gain.record_query`, `output.default`). Stored in `.repoctx/index.db` schema v3 settings table. Precedence: CLI flag → env var → settings → default.
