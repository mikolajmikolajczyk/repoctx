...
- `repoctx import-cycles` / `modules` (unreleased, epic #4) — petgraph over the import graph: circular-import detection (SCC) + resolved import topology with dependency-first build order (toposort). Resolution (#8): JS/TS relative + tsconfig aliases, **Rust `crate`/`self`/`super` module paths**; external crates/packages + Python/Go specifiers stay external. First petgraph adopter (ephemeral graph, decision 2026-06-16).
