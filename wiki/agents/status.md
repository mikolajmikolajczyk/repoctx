...
- `repoctx overview` (unreleased, issue #5) — repo architecture in one call: totals (code vs doc/config split), per-language, per-directory module sizes **ranked by code symbols** (markdown headings + config keys excluded — #9-D), entry points (`main` + JS/TS bootstraps), hotspots (most-called symbols, receiver-aware). Composes index + call graph; no new extraction. Public-API surface deferred (#8).
