...
Repo-specific notes for coding agents (Claude Code, Codex, opencode, Cursor, Aider, Copilot, …). Generic software-engineering advice is out of scope. End users wire `repoctx` into their own agent with `repoctx init` (installs guidance and, for Claude, a SessionStart hook that runs `repoctx prime`) — see [`wiki/user/init.md`](wiki/user/init.md).
...
|------|------|--------------|
| **Source of truth for roadmap, milestones, backlog** | GitHub issues — `gh issue list` | Always. **Don't read roadmaps from markdown.** |
...
| Working on issues (state labels, branch naming, PR flow, session handoff) | [`wiki/agents/working-on-issues.md`](wiki/agents/working-on-issues.md) | Before picking up a GitHub issue |
...
**GitHub is the canonical forge** — <https://github.com/mikolajmikolajczyk/repoctx>. Code, pull requests (`gh pr create`), and issues + roadmap (`gh issue list`) all live there. Push code with `git push origin main`. Issue labels: `state:*`, `priority:*`, `milestone:*`, `epic`, `blocked:*`. See [`wiki/agents/working-on-issues.md`](wiki/agents/working-on-issues.md) for the full lifecycle.
...
- **Don't pre-empt later milestones.** If something carries a future `milestone:*` GitHub label, don't half-implement it while working on the current one.
